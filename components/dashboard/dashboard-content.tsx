...
import { markEpisodeWatchedAction, getTVShowsAction } from "@/app/actions/tvshows"
...
import { PhevStats, CarSummary } from "@/types/phev"
...
interface DashboardContentProps {
    ...
    initialPHEVStats: PhevStats | null
    initialPHEVCarSummaries: CarSummary[]
    initialJobStats: JobStats | null
}
...
export function DashboardContent({
    ...
    const [phevStats, setPHEVStats] = React.useState<PhevStats | null>(initialPHEVStats)
    const [phevCarSummaries, setPHEVCarSummaries] = React.useState<CarSummary[]>(initialPHEVCarSummaries)
    const [jobStats, setJobStats] = React.useState<JobStats | null>(initialJobStats)
    ...
    const totalBookMinutes = activeBooks.reduce((total, book) => total + (book.minutes || 0), 0)
    ...
    // Derive status from progress (same logic as TV shows page)
    const watchingShows = tvshows.filter(show => {
        // Skip stopped shows
        if (show.status === 'Stopped') return false
        const progress = show.totalEpisodes > 0 ? (show.watchedEpisodes / show.totalEpisodes) * 100 : 0
        // Show is "watching" if progress > 0 and < 100
                ...
                primaryFields={detailItem && detailType ? (
                    detailType === 'game' ? [
                        ...
                        detailType === 'book' ? [
                            ...
                            { label: "Time", value: `${detailItem.hours || 0}h ${detailItem.minutes || 0}m`, icon: <Clock className="h-4 w-4" /> },

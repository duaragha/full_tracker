...
export async function addContainer(container: Omit<Container, 'id' | 'createdAt' | 'updatedAt'>): Promise<Container> {
  const result = await pool.query<any>(
    ...
    [
      ...
      container.brand || null,
      container.model || null,
      container.material || null,
      container.size || null,
      container.capacity || null,
      container.purchasedDate || null,
      container.purchasedFrom || null,
      container.cost !== undefined ? container.cost : null,
...
export async function updateContainer(id: number, container: Partial<Container>): Promise<void> {
  await pool.query(
    ...
    [
      ...
      container.condition || null,
      container.notes || null,
      container.isOwned !== undefined ? container.isOwned : null,
      id,
    ]
  )
}
